...
- All your project code into a single main package (`p2k2_converter/`)
...
```bash
<root directory>
├── p2k2_converter/             # main package (should be named after your project)
...
1. Use this template to create a new GitHub repository, say `p2k2_converter`
...
2. Clone the `p2k2_converter` repository

3. Open a shell into your local `p2k2_converter` directory and run 
    ```bash
    ./rename-template.sh p2k2_converter
    ``` 
    
    This will coherently rename the template's project name with the one chosen by you (i.e. `p2k2_converter`, in this example)
...
13. Put your main (resp. test) code in `p2k2_converter/` (resp. `test/`)

## How to do stuff

### Run your code as an application

This will execute the file `p2k2_converter/__main__.py`:
```bash
python -m p2k2_converter 
```
...
GitHub actions automatically release a new version of `p2k2_converter` on PyPi whenever commits are pushed on either the `main`/`master` or `develop` branches, as well as when new tags are pushed.
